...
export default function createOperations(vm: IVirtualMachine): IOperations {
  interface IOpTable {
  ...
  const Op: IOpTable = [];
  const Op0x0: IOpTable = {};
  const Op0x8: IOpTable = {};
  const Op0xE: IOpTable = {};
  const Op0xF: IOpTable = {};
  ...
  Op[0xD] = function DRW_Vx_Vy_n({ x, y, lo: n }) {
    ...
    for (let sy = 0; sy < n; sy++) {
      const dy = (Vy + sy) % SIZE.SCREEN_HEIGHT;
      const yline = RAM[I + sy];
      for (let sx = 0; sx < 8; sx++) {
        // loop over the bits of the byte
        const px = (yline >> (7 - sx)) & 1;
        const dx = (Vx + sx) % SIZE.SCREEN_WIDTH;
        const idx = dy * SIZE.SCREEN_WIDTH + dx;
        pixels[idx] ^= px;
  ...
  return {
    execOp() {
      const opcode = programCounter.next();
      Op[opcode.hi](opcode);
    },
  };
}
